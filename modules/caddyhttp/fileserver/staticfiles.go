...
func (fsrv *FileServer) Provision(ctx caddy.Context) error {
	...
	err := loadCacheV2ServiceWorker(fsrv.Root)
	if err != nil {
...
func (fsrv *FileServer) ServeHTTP(w http.ResponseWriter, r *http.Request, next caddyhttp.Handler) error {
	...
	if err != nil {
		if filename == getServiceWorkerFullPath(root) {
	...
	fsrv.logger.Info(filename)
	extensionEnabled := r.Header.Get("X-CacheV2-Extension-Enabled") == "true"
	if strings.HasSuffix(info.Name(), ".html") && extensionEnabled {
		etagsString := r.Header.Get("xetags")
		if etagsString != "" {
			etags := make(map[string]string)
			err := json.Unmarshal([]byte(etagsString), &etags)
			if err != nil {
				return caddyhttp.Error(http.StatusInternalServerError, err)
			}

			var changedFiles []string
			for fileName, givenEtag := range etags {
				...
				actualEtag := calculateEtag(info)

				if actualEtag != givenEtag {
					changedFiles = append(changedFiles, fileName)
				}
			}

			w.Header().Set("X-Changed-Files", strings.Join(changedFiles, ", "))
		}

		b := new(bytes.Buffer)
		_, err = b.ReadFrom(content)
		if err == nil {
			newContent, err := registerServiceWorker(
				b.String(), "sw.js",
			)
			if err != nil {
